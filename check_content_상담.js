...
async function check() {
    ...
    const missed = records.filter(r => 
        ...
        (r.content?.includes('상담') || r.content?.includes('clovanote') || r.content?.includes('요약'))
    );
    
    console.log("Positive records with '상담' in content but NOT in category:", missed.length);
    missed.forEach(r => {
        console.log(`ID: ${r.id}, Cat: "${r.category}", Content: ${r.content.substring(0, 100)}...`);
    });
}
